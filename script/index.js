...
confirmBtn.addEventListener('click',function(){
    ...
    // 할일을 적은 상태일경우
    if(writeContents.value != ''){//tectarea의 값이 빈 값이 아닌 경우 참
        const allCon = document.querySelector('#all_con'); //ol 선텍(자식요소삽입위해)
        const li = document.createElement('li'); //생성위치(클릭이벤트안)
        const dataA = document.createElement('a'); //클릭할때마다 생성
        const a = document.createElement('a'); //클릭할때마다 생성
        
        dataA.classList.add('date');//기존css의 디자인통일을 위해서 같은이름등록
        li.classList.add = ('contents4');//기존 클래스 일치해서 디자인 적용목적
        li.classList.add = ('contents');//기존 클래스 일치해서 디자인 적용목적

        dataA.textContent = '04.03';//오늘 날짜 대입(문자열)
        dataA.href = '#'; //a태그 생성시 속성없이 기본생성<a>, href속성추가대입
        a.innerHTML = writeContents.value;//create생성한게 아닌 기존태그값을 대입

        li.appendChild(dataA);//li 부모안 마지막 자식 위치에 날짜 A 삽입(먼저 시작하는 순서)
